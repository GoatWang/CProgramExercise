...
bool hists_same(const int h1[MAXTHROW], const int h2[MAXTHROW])
{
    // int hofh1[MAXTHROW]; //histogram of histogram
    // int hofh2[MAXTHROW]; //histogram of histogram
    // set_array(hofh1, MAXTHROW, 0);
    // set_array(hofh2, MAXTHROW, 0);
    // for (int i=0; i<MAXTHROW; i++){
    //     hofh1[h1[i]] += 1;
    //     hofh2[h2[i]] += 1;
    // }
...
bool isfullhouse(const int d[NUMDICE])
{
    bool two_exist = false, three_exist = false;
    int h[MAXTHROW]; makehist(d, h);
    for (int i=0; i<MAXTHROW; i++){
        if (h[i] == 2){
            two_exist = true;        
        }
    ...
    return two_exist && three_exist;
}

/* Does this hand have 4 lots of one number and 1 of another ? */
bool is4ofakind(const int d[NUMDICE])
{
    bool one_exist = false, four_exist = false;
    int h[MAXTHROW]; makehist(d, h);
    for (int i=0; i<MAXTHROW; i++){
        if (h[i] == 1){
            one_exist = true;        
